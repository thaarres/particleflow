...
class GravNetClusteringModel(tf.keras.Model):
    def __init__(self, n_neighbours=40, n_dimensions=4, n_filters=42, n_propagate=18, **kwargs):
        ...
        for i in range(4):
            ...
            dense0 = self.add_layer(tf.keras.layers.Dense, 64, activation='tanh', name='dense_%d-0' % i)
            dense1 = self.add_layer(tf.keras.layers.Dense, 64, activation='tanh', name='dense_%d-1' % i)
            dense2 = self.add_layer(tf.keras.layers.Dense, 64, activation='tanh', name='dense_%d-2' % i)

            gravnet = self.add_layer(GravNet, n_neighbours, n_dimensions, n_filters, n_propagate, name='gravnet_%d' % i)

            batchnorm = self.add_layer(tf.keras.layers.BatchNormalization, momentum=momentum, name='batchnorm_%d' % i)

            self.blocks.append((gex, dense0, dense1, dense2, gravnet, batchnorm))

        self.output_dense_0 = self.add_layer(tf.keras.layers.Dense, 128, activation='relu', name='output_0')
        self.output_dense_1 = self.add_layer(tf.keras.layers.Dense, 3, activation='relu', name='output_1')
...
class GarNetClusteringModel(tf.keras.Model):
    def __init__(self, aggregators=([4] * 11), filters=([32] * 11), propagate=([20] * 11), **kwargs):
        ...
        self.input_batchnorm = self.add_layer(tf.keras.layers.BatchNormalization, momentum=momentum, name='input_batchnorm')
        self.input_dense = self.add_layer(tf.keras.layers.Dense, 32, activation='tanh', name='input_dense')
        ...
        self.output_dense_0 = self.add_layer(tf.keras.layers.Dense, 48, activation='relu', name='output_0')
        self.output_dense_1 = self.add_layer(tf.keras.layers.Dense, 3, activation='relu', name='output_1')
